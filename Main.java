...
class Game {
    public static class Controller {
        ...
        // Modify the reset method to pass the playerName to dialog methods
    ...
    public static class View extends JPanel {
        final JLabel tries;
        final JTextField playerNameField; //declare player name field here
        ...
        public View(Model model) {
            ...
            JPanel playerNamePanel = new JPanel(); // Create a panel to hold the label and text field
            playerNamePanel.setLayout(new FlowLayout(FlowLayout.CENTER)); // Use FlowLayout to center-align elements

            JLabel playerNameLabel = new JLabel("Player Name: ");
            playerNameLabel.setFont(new Font("MV Boli", Font.PLAIN, 16));

            playerNameField = new JTextField(20);
            playerNameField.setFont(new Font("MV Boli", Font.PLAIN, 16));

            // Add the label and text field to the playerNamePanel
            playerNamePanel.add(playerNameLabel);
            playerNamePanel.add(playerNameField);

            // Add the playerNamePanel to the view
    ...
    public static class MemoryButton extends JButton {
        ...
        static final Image NO_IMAGE = Utilities.loadImage("9.png");
    ...
    public static class Dialogs {
        public static void showLoseDialog(JFrame window, String playerName) {
            String message = String.format("you lost!!, %s" , playerName);
            UIManager.put("OptionPane.background", new Color(0XEA99D5));
            UIManager.put("Panel.background", new Color(0XEA99D5));
            JOptionPane.showMessageDialog(window.getContentPane(), message ," you lost, try again!", JOptionPane.INFORMATION_MESSAGE);
        }
        // Modify showWinDialog and showLoseDialog methods
        public static void showWinDialog(JFrame window, Model model, String playerName) {
        String message = String.format("Congratulations, %s! You won!!", playerName);
        UIManager.put("OptionPane.background", new Color(0XEA99D5));
        UIManager.put("Panel.background", new Color(0XEA99D5));
        JOptionPane.showMessageDialog(window.getContentPane(), message, "Congratulations!", JOptionPane.INFORMATION_MESSAGE);
        }
    ...
    public static class ButtonActionListener implements ActionListener {
        ...
        // Add a field to store the player's name
        private String playerName = "";
        static JButton lastDisabledButton = null;
        static final Image TRAP_IMAGE = Utilities.loadImage("9.png");
        ...
        public ButtonActionListener(Controller controller) {
            ...
            this.trap = new ReferencedIcon(TRAP_IMAGE, "9.png");
        }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            playerName = view.playerNameField.getText();
            // In the actionPerformed method, add this code to capture the player's name
            if (!model.isGameStarted()) {
            // playerName = view.playerNameField.getText(); // Capture the player's name
            ...
            if (enabledButtons.size() == 0) { // If all the buttons are disabled
                Dialogs.showWinDialog(window, model,playerName);
                controller.reset(new Model(controller.getModel().getColumns())); // Reset the game
                // Dialogs.showWinDialog(window, model,playerName); // Show the win dialog
            }
            lastDisabledButton = button;    // Store the last button
            if (model.getTries() == 0) { // If the number of tries is 0
                controller.reset(new Model(controller.getModel().getColumns())); // Reset the game
                Dialogs.showLoseDialog(window,playerName); // Show the lose dialog
